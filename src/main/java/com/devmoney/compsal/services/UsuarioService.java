...
public class UsuarioService {
	...
	public List<UsuarioNew> findAllByOrderByIdDesc() {
		return repo.findAllByOrderByIdDesc();
	}

	public List<UsuarioNew> findAllByOrderByNomeDesc() {
		return repo.findAllByOrderByNomeDesc();
	}

	public List<UsuarioNew> findAllByOrderByNomeAsc() {
	...
	public String deleteVerifica(Integer id) {
		...
		try {
			if (repoTime.findByGoleiro(id) != null) {
				...
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Ala Esquerda no Time "
						+ num2.getNome();
				return msg;
				...
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Jogador Reserva 1 no Time "
						+ num2.getNome();
				return msg;
			} else if (repoTime.findByJogadorReserva2(id) != null) {
				num2 = repoTime.findByJogadorReserva2(id);
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Jogador Reserva 2 no Time "
						+ num2.getNome();
				return msg;
			} else if (repoTime.findByJogadorReserva3(id) != null) {
				num2 = repoTime.findByJogadorReserva3(id);
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Jogador Reserva 3 no Time "
						+ num2.getNome();
				return msg;
			} else if (repoTime.findByJogadorReserva4(id) != null) {
				num2 = repoTime.findByJogadorReserva4(id);
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Jogador Reserva 4 no Time "
						+ num2.getNome();
				return msg;
			} else if (repoTime.findByJogadorReserva5(id) != null) {
				num2 = repoTime.findByJogadorReserva5(id);
				System.err.println(num2.getNome());
				msg = "O usuário não pode ser excluído pois está cadastrado como Jogador Reserva 5 no Time "
						+ num2.getNome();
				return msg;
				...
				return msg;
			} else if (repoJogo.findByAnotador(id).size() > 0) {
	...
	public UsuarioNew fromDTO(UsuarioDTO objDto) {
		return new UsuarioNew(null, objDto.getNome(), objDto.getCpf(), objDto.getApelido(), objDto.getSexo(),
				objDto.getDtNascimento(), objDto.getTelefone(), objDto.getEmail(), objDto.getEndereco(),
				objDto.getNumeroEnd(), objDto.getCep(), objDto.getBairro(), objDto.getMunicipio(), objDto.getUf());
	}
